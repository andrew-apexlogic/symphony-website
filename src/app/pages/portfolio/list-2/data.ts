...
export const portfolioList2: PortfolioList2Type[] = [
  {
    ...
    title: 'Data Extraction and Integrations',
    description:
      '<p class="fs-sm pb-3 pb-lg-4 mb-3">Symphony was designed to work in your ecosystem and therefore,  API First — Every action taken by users in Symphony makes a RESTful API call.  The integrations do not have to be read only.  A possible use case may be that an external system needs to post an order to the contract vehicle.  The data points and documents retained by the third party system can generate, post, and release an order in Symphony automatically without any direct interaction with the Symphony user interface. Finally, integration facilitates the enhancement of external dashboards and reports that are already in use with data from Symphony. All data collected in Symphony is accessible via the program interface or through requested reporting.  The final contract files containing all the documents uploaded by industry as part of their offer, award documents, and evaluation documents can be provided to a specified System of Record.</p>',
    awards: [
